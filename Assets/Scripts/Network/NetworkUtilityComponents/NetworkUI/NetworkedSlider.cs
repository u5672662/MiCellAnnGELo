...
namespace XRMultiplayer
{
    ...
    public class NetworkedSlider : NetworkBehaviour
    {
        ...
        // Local value used when no network session is active.
        ...
        private void Awake()
        {
            ...
            _localValue = m_StartValue;

            var isListening = NetworkManager.Singleton != null && NetworkManager.Singleton.IsListening;
            var writePermission = isListening
                ? NetworkVariableWritePermission.Server
                : NetworkVariableWritePermission.Owner;

            m_NetworkSliderValue = new NetworkVariable<float>(m_StartValue,
                NetworkVariableReadPermission.Everyone, writePermission);

            if (!isListening)
            {
                m_Slider.SetValueWithoutNotify(_localValue);
            }
        }

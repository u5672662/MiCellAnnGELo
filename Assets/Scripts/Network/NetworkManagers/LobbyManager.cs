...
namespace XRMultiplayer
{
    ...
    public class LobbyManager : MonoBehaviour
    {
        ...
        async Task<Allocation> CreateRelayAllocationAsync(int maxPlayers)
        {
            try
            {
                ...
                {
                    return await RelayService.Instance.CreateAllocationAsync(maxPlayers, regions[0].Id);
                }
            }
            catch (Exception e)
            {
                Utils.Log($"{k_DebugPrepend}Failed to get regions: {e.Message}", 1);
            }
        ...
        public async Task<Lobby> CreateLobby(string roomName = null, bool isPrivate = false, int playerCount = XRINetworkGameManager.maxPlayers)
        {
            try
            {
                ...
                var alloc = await CreateRelayAllocationAsync(XRINetworkGameManager.maxPlayers);

...
namespace XRMultiplayer
{
    ...
    public class PlayerOptions : MonoBehaviour
    {
        ...
        {
            if (panelID < 0 || panelID >= m_Panels.Length)
            {
                Debug.LogWarning($"TogglePanel called with invalid index {panelID}");
                return;
            }

            for (int i = 0; i < m_Panels.Length; i++)
            {
                if (i < m_PanelToggles.Length)
        ...
        {
            if(m_MoveProvider.TryGetComponent<GravityProvider>(out var gravityProvider))
            {
